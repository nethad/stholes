...
public class CompleteCanvas extends JComponent {
	...
	final static BasicStroke dashed = new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, dash1,
			0.0f);
	...
	private void drawRectangles(Graphics g) {
		if (this.rectangles != null) {
			for (DrawableRectangle dr : this.rectangles) {
				g.setColor(dr.color);
				g.drawRect(dr.rectangle.x, dr.rectangle.y, dr.rectangle.width, dr.rectangle.height);
				g.drawString(dr.labelText, dr.rectangle.x + LABEL_X_PADDING, dr.rectangle.y + LABEL_Y_PADDING);
			}
		}
	}
	...
	public void setRootBucket(Bucket bucket) {
		...
		DrawableRectangle drawableRectangle = new DrawableRectangle(scaleRectangle(bucket.getRectangle()),
				String.valueOf(bucket.getFrequency()), BUCKET_COLOR);
	...
	public void addAdditionalBucket(Bucket bucket, Color color) {
		DrawableRectangle drawableRectangle = new DrawableRectangle(scaleRectangle(bucket.getRectangle()),
				String.valueOf(bucket.getFrequency()), color);
		this.rectangles.add(drawableRectangle);
		repaint();
	}
	...
	private Point scalePoint(Point2D.Double dataPoint) {
		return new Point((int) (PADDING + (dataPoint.x * SCALE_FACTOR)), (int) (PADDING + (dataPoint.y * SCALE_FACTOR)));
	}

	private void addRectangles(List<Bucket> buckets) {
		for (Bucket childBucket : buckets) {
			DrawableRectangle drawableRectangle = new DrawableRectangle(scaleRectangle(childBucket.getRectangle()),
